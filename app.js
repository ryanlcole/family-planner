...
let g=grocery(),gd=g.filter(x=>state.purchased[x.key]).length;$("#purchasePct").textContent=(g.length?Math.round(gd/g.length*100):0)+"%";let expectedBasket=g.reduce((s,x)=>s+(x.expected||0),0),actualBasket=g.reduce((s,x)=>s+(state.purchaseCost[x.key]===""||state.purchaseCost[x.key]==null?0:n(state.purchaseCost[x.key])),0),actualEntered=g.some(x=>state.purchaseCost[x.key]!==""&&state.purchaseCost[x.key]!=null),delta=actualEntered?expectedBasket-actualBasket:null;$("#shoppingStats").innerHTML=[["Expected",precise(expectedBasket)],["Actual entered",actualEntered?precise(actualBasket):"—"],["Savings / increase",delta==null?"—":(delta>=0?precise(delta)+" saved":precise(Math.abs(delta))+" over")]].map(x=>'<div class="stat"><small>'+x[0]+'</small><div class="value">'+x[1]+'</div></div>').join("");$("#shopping").innerHTML=g.map(x=>{let hit=dealForIngredient(x.name,x.unit),d=hit?.deal,dealLabel=d?(d.type==="price"?money(d.value):d.type==="percent"?d.value+"% off":money(d.value)+" off"):"";return `<div class="row"><div class="tools"><input class="check buy" data-k="${encodeURIComponent(x.key)}" type="checkbox" ${state.purchased[x.key]?"checked":""}><div><b>${esc(x.name)}</b><div class="small">Need ${Math.round(x.qty*100)/100} ${esc(x.unit)} · on hand ${Math.round(x.have*100)/100} · buy ${Math.round(x.buy*100)/100} · expected ${x.expected==null?"price needed":money(x.expected)} · ${[...x.sources].join(" · ")}${d?" · coupon/deal "+esc(dealLabel)+" @ "+esc(hit.store||"Any"):" · no saved coupon match"}</div></div></div><div class="tools"><input class="field paid" style="width:92px" data-k="${encodeURIComponent(x.key)}" type="number" step=".01" placeholder="Paid" value="${state.purchaseCost[x.key]??""}">${d?.sourceUrl?`<a class="btn" href="${esc(d.sourceUrl)}" target="_blank" rel="noopener">Coupon</a>`:""}<a class="btn" href="${search(x.name+" coupon sale "+state.zip)}" target="_blank" rel="noopener">Find coupon</a><a class="btn" href="${search(x.name+" grocery price "+state.zip)}" target="_blank" rel="noopener">Price</a></div></div>`}).join("")||`<div class="empty">Inventory covers current needs, or no essentials/meals are planned.</div>`;

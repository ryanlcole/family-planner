...
function importBulkStock(raw){
  const lines=String(raw||"").split(/\r?\n/).map(x=>x.trim()).filter(Boolean);
  let added=0,merged=0,skipped=0;
  for(const line of lines){
    if(line.startsWith("#"))continue;
    const p=line.split("|").map(x=>x.trim());
    const qty=Math.max(0,n(p[0])),name=p[1]||"";
    if(!name||qty<=0){skipped++;continue}
    const category=p[2]||"Household",aisle=p[3]||"",threshold=p[4]===""||p[4]==null?null:Math.max(0,n(p[4])),unit=p[5]||"item";
    let item=state.inventory.find(i=>canon(i.name)===canon(name));
    if(item){
      item.qty=n(item.qty)+qty;item.onHand=item.qty>0;
      if(!item.category)item.category=category;if(!item.aisle)item.aisle=aisle;if(!item.unit)item.unit=unit;
      if((item.minQty===""||item.minQty==null)&&threshold!=null)item.minQty=threshold;
      if(item.shoppingState&&n(item.qty)>n(item.minQty)){item.shoppingState="";item.buyQty="";item.cartProduct="";item.cartStore="";item.cartPrice=""}
      merged++;
    }else{
      state.inventory.push({id:"inv"+Date.now()+Math.random().toString(36).slice(2,6),name,category,aisle,onHand:true,qty,unit,minQty:threshold??0,shoppingState:"",buyQty:""});
      added++;
    }
  }
  state.lastStockImportAt=new Date().toISOString();
  save();
  return{added,merged,skipped,total:added+merged};
}
const inventoryWarning=i=>n(i.minQty)>0&&n(i.qty)<=n(i.minQty)&&!["list","cart"].includes(i.shoppingState||"");
const inventoryWorkflowRows=()=>state.inventory.map((item,index)=>({item,index})).sort((a,b)=>{
  const aw=inventoryWarning(a.item)?0:1,bw=inventoryWarning(b.item)?0:1;
  return aw-bw||String(a.item.name||"").localeCompare(String(b.item.name||""));
});
const purchaseRowsFor=i=>(state.purchaseHistory||[]).filter(h=>h.inventoryId===i.id||(!h.inventoryId&&canon(h.item)===canon(i.name))).sort((a,b)=>String(b.date||"").localeCompare(String(a.date||""))||n(b.createdAt)-n(a.createdAt));
function favoriteProductFor(i){
  const h=purchaseRowsFor(i).find(x=>x.rating==="favorite"&&String(x.product||"").trim());
  return h?.product||i.favoriteProduct||"";
}
function alternativeProductFor(i){
  const fav=canon(favoriteProductFor(i));
  const h=purchaseRowsFor(i).find(x=>x.rating==="acceptable"&&String(x.product||"").trim()&&canon(x.product)!==fav);
  return h?.product||i.alternativeProduct||"";
}
function knownWorkflowPrice(i){
  try{
    const p=pricedProduct(i.name,i.unit||"item");if(!p)return null;
    const temp={...p,need:Math.max(1,n(i.buyQty)||n(i.minQty)||1)};
    const xs=STORE_NAMES.map(store=>({store,cost:offerCost(temp,store)})).filter(x=>Number.isFinite(x.cost)).sort((a,b)=>a.cost-b.cost);
    return xs[0]||null;
  }catch{return null}
}
function workflowListRows(){
  let rows=state.inventory.map((item,index)=>({item,index})).filter(x=>x.item.shoppingState==="list");
  const mode=state.shoppingSort||"need";
  rows.sort((a,b)=>{
    if(mode==="aisle")return String(a.item.aisle||"ZZZ").localeCompare(String(b.item.aisle||"ZZZ"))||String(a.item.name||"").localeCompare(String(b.item.name||""));
    if(mode==="type")return String(a.item.category||"Other").localeCompare(String(b.item.category||"Other"))||String(a.item.aisle||"").localeCompare(String(b.item.aisle||""))||String(a.item.name||"").localeCompare(String(b.item.name||""));
    const au=Math.max(0,n(a.item.minQty)-n(a.item.qty)),bu=Math.max(0,n(b.item.minQty)-n(b.item.qty));
    return bu-au||String(a.item.name||"").localeCompare(String(b.item.name||""));
  });
  return rows;
}
function moveInventoryToList(i){
  const item=state.inventory[i];if(!item)return;
  item.shoppingState="list";
  item.buyQty=Math.max(1,n(item.minQty)||1);
  item.cartProduct=item.cartProduct||favoriteProductFor(item)||item.name||"";
  render();
}
function moveInventoryToCart(i){
  const item=state.inventory[i];if(!item)return;
  item.shoppingState="cart";
  item.buyQty=Math.max(1,n(item.buyQty)||n(item.minQty)||1);
  item.cartProduct=item.cartProduct||favoriteProductFor(item)||item.name||"";
  render();
}
function returnCartToList(i){const item=state.inventory[i];if(item){item.shoppingState="list";render()}}
function completeCartPurchase(i){
  const item=state.inventory[i];if(!item)return;
  const qty=Math.max(0,n(item.buyQty));
  if(qty<=0)return;
  state.purchaseHistory=state.purchaseHistory||[];
  state.purchaseHistory.unshift({
    id:"ph"+Date.now()+Math.random().toString(36).slice(2,6),
    inventoryId:item.id,item:item.name||"Item",product:item.cartProduct||favoriteProductFor(item)||item.name||"Item",
    qty,unit:item.unit||"item",store:item.cartStore||"",price:item.cartPrice===""?"":n(item.cartPrice),
    rating:"",date:new Date().toISOString().slice(0,10),createdAt:Date.now()
  });
  item.qty=n(item.qty)+qty;item.onHand=n(item.qty)>0;item.shoppingState="";item.buyQty="";item.cartProduct="";item.cartStore="";item.cartPrice="";
  state.shoppingView="history";
  render();
}
function renderShoppingWorkflow(){
  state.shoppingView=state.shoppingView||"list";state.shoppingSort=state.shoppingSort||"need";state.purchaseHistory=Array.isArray(state.purchaseHistory)?state.purchaseHistory:[];
  const list=state.inventory.filter(i=>i.shoppingState==="list"),cart=state.inventory.filter(i=>i.shoppingState==="cart"),warnings=state.inventory.filter(inventoryWarning);
  $("#cartCount").textContent=cart.length;
  $("#shoppingWorkflowStats").innerHTML=[["Warnings",warnings.length],["Shopping list",list.length],["Cart",cart.length],["History",state.purchaseHistory.length]].map(x=>'<div class="stat"><small>'+x[0]+'</small><div class="value">'+x[1]+'</div></div>').join("");
  $(".shopTab").forEach(x=>x.classList.toggle("primary",x.dataset.shopTab===state.shoppingView));
  $(".shopPane").forEach(x=>x.style.display=x.dataset.shopPane===state.shoppingView?"block":"none");
  $("#shoppingSort").value=state.shoppingSort;
  $("#shoppingListRows").innerHTML=workflowListRows().map(({item:i,index:k})=>{
    const fav=favoriteProductFor(i),alt=alternativeProductFor(i),kp=knownWorkflowPrice(i),q=Math.max(1,n(i.buyQty)||n(i.minQty)||1);
    const query=(fav||i.name)+" "+(alt?alt+" ":"")+"best price store brand equivalent "+(state.zip||"");
    return '<div class="card workflowItem"><div class="split"><div><h3>'+esc(i.name||"Item")+'</h3><div class="small">'+esc(i.category||"Other")+' · '+esc(i.aisle||"Aisle not set")+' · need '+q+' '+esc(i.unit||"item")+'</div></div><label class="tools"><input class="check toCart" data-i="'+k+'" type="checkbox"> In cart</label></div><div class="grid g2 section"><label><span>Qty to buy</span><input class="field workflowBuyQty" data-i="'+k+'" type="number" min="0" step="1" value="'+q+'"></label><div><span class="small">Recommendation</span><div><b>'+(fav?esc(fav):'No favorite yet')+'</b></div><div class="small">Alternative: '+(alt?esc(alt):'No acceptable alternative saved yet')+'</div>'+(kp?'<div class="small greenText">Known price: '+esc(kp.store)+' · '+money(kp.cost)+'</div>':'')+'</div></div><div class="tools section"><a class="btn primary" href="'+search(query)+'" target="_blank" rel="noopener">Search best prices</a><a class="btn" href="'+search((i.name||"item")+" store brand equivalent "+(state.zip||""))+'" target="_blank" rel="noopener">Find off-brand</a><button class="btn danger workflowRemoveList" data-i="'+k+'">Remove from list</button></div></div>';
  }).join("")||'<div class="empty">Nothing on the shopping list. Inventory warnings stay in Inventory until you choose Add to list.</div>';
  $("#cartRows").innerHTML=state.inventory.map((item,index)=>({item,index})).filter(x=>x.item.shoppingState==="cart").map(({item:i,index:k})=>{
    const q=Math.max(1,n(i.buyQty)||1);
    return '<div class="card workflowItem"><div class="split"><div><h3>'+esc(i.name||"Item")+'</h3><div class="small">'+esc(i.category||"Other")+' · '+esc(i.aisle||"Aisle not set")+'</div></div><button class="btn cartBack" data-i="'+k+'">Back to list</button></div><div class="grid g2 section"><label><span>What is actually in the cart?</span><input class="field cartProduct" data-i="'+k+'" value="'+esc(i.cartProduct||favoriteProductFor(i)||i.name||"")+'"></label><label><span>Store</span><input class="field cartStore" data-i="'+k+'" value="'+esc(i.cartStore||"")+'" placeholder="Store"></label><label><span>Quantity</span><div class="tools"><button class="btn cartMinus" data-i="'+k+'">−</button><input class="field cartQty" style="width:90px" data-i="'+k+'" type="number" min="0" step="1" value="'+q+'"><button class="btn cartPlus" data-i="'+k+'">+</button></div></label><label><span>Price now / optional</span><input class="field cartPrice" data-i="'+k+'" type="number" step=".01" value="'+(i.cartPrice??"")+'" placeholder="Can enter later"></label></div><button class="btn green section cartPurchased" data-i="'+k+'">Purchased + put away</button></div>';
  }).join("")||'<div class="empty">Your cart is empty. Check “In cart” on the Shopping list as you pick items up.</div>';
  $("#purchaseHistoryRows").innerHTML=state.purchaseHistory.map((h,k)=>'<div class="card workflowItem"><div class="split"><div><b>'+esc(h.item||"Item")+'</b><div class="small">'+esc(h.date||"")+' · '+n(h.qty)+' '+esc(h.unit||"item")+'</div></div><button class="btn historyBuyAgain" data-i="'+k+'">Buy again</button></div><div class="grid g2 section"><label><span>Product purchased</span><input class="field histProduct" data-i="'+k+'" value="'+esc(h.product||"")+'"></label><label><span>Store</span><input class="field histStore" data-i="'+k+'" value="'+esc(h.store||"")+'"></label><label><span>Price</span><input class="field histPrice" data-i="'+k+'" type="number" step=".01" value="'+(h.price??"")+'"></label><label><span>Rating</span><select class="field histRating" data-i="'+k+'"><option value="" '+(!h.rating?'selected':'')+'>Not rated</option><option value="favorite" '+(h.rating==="favorite"?'selected':'')+'>★ Favorite</option><option value="acceptable" '+(h.rating==="acceptable"?'selected':'')+'>Acceptable</option><option value="dislike" '+(h.rating==="dislike"?'selected':'')+'>Dislike</option></select></label></div></div>').join("")||'<div class="empty">Purchases will appear here after you finish the Cart.</div>';
  const g=grocery();
  $("#plannerNeeds").innerHTML=g.map(x=>'<div class="row"><div><b>'+esc(x.name)+'</b><div class="small">Need '+Math.round(x.buy*100)/100+' '+esc(x.unit)+' · '+[...x.sources].join(" · ")+'</div></div><div class="tools"><b>'+(x.expected==null?'price needed':money(x.expected))+'</b><a class="btn" href="'+search(x.name+" best price "+(state.zip||""))+'" target="_blank" rel="noopener">Search</a></div></div>').join("")||'<div class="empty">No recipe or monthly-plan shortages right now.</div>';
}
...
<section class="view" data-v="food"><div class="panel"><div class="split"><div><div class="eyebrow">Food budget</div><h2>EBT + inventory + store comparison</h2><p class="muted">Monthly benefit and card balance are different. The monthly benefit is the amount planned for this month; the actual EBT card balance can include carryover and only changes when purchases post. Planned essentials and selected meals reduce the plan, not the card balance.</p></div><label class="stat" style="display:block;min-width:250px"><small>EBT card balance</small><div style="display:flex;align-items:center;gap:7px;margin-top:7px"><span class="value greenText" style="font-size:32px">$</span><input class="field" id="ebtCardBalance" inputmode="decimal" type="number" min="0" step=".01" placeholder="0.00" aria-label="Current EBT card balance" style="font-size:32px;font-weight:850;color:var(--green);padding:4px 6px;min-height:52px;width:100%"></div></label></div><div class="grid g4 stats section" id="foodBudgetStats"></div><label class="section" style="max-width:260px"><span>Dinner slots / month</span><input class="field" id="dinnerSlots" type="number" min="1" step="1"></label><div class="notice section"><b>Plan vs. card</b><div class="small" id="ebtBalanceMeta">Card balance is tracked separately from the monthly meal plan.</div></div></div><div class="card section"><div class="split"><div><h2>Inventory</h2><div class="small">Track the generic item, not the brand. Lower Qty when you open or use the next one. When Qty reaches Threshold, the warning rises to the top. Check Add to list when you actually want to shop for it.</div></div><button class="btn" id="addInventory">+ Inventory item</button></div><div class="grid g4 stats section" id="inventoryStats"></div><div class="tablewrap section"><table class="inventoryWorkflow"><thead><tr><th>Qty</th><th>Threshold</th><th>Warning</th><th>Item</th><th>Type</th><th>Aisle / section</th><th>Add to list</th><th></th></tr></thead><tbody id="inventoryRows"></tbody></table></div></div><div class="card section"><div class="split"><div><h2>Bulk stock import</h2><div class="small">Paste one item per line as: Qty | Item | Type | Aisle / section | Threshold | Unit. Existing generic items are merged and quantities are increased.</div></div></div><textarea class="field section" id="bulkStockText" rows="7" placeholder="2 | Toothpaste | Personal care | Dental | 1 | tube"></textarea><div class="tools section"><button class="btn primary" id="bulkStockApply">Add to inventory</button><button class="btn" id="bulkStockClear">Clear</button></div><div class="small section" id="bulkStockStatus"></div></div><div class="card section"><div class="split"><div><h2>Shopping strategy</h2><div class="small">Automatically minimize the basket with one store, two stores, or the lowest price for every item.</div></div><select class="field" id="shoppingMode" style="max-width:260px"><option value="lowest">Lowest price per item</option><option value="one">Best one-store shop</option><option value="two">Best two-store shop</option></select></div><div id="shopOptimizeSummary" class="notice section"></div><div id="optimizedBasket" class="section"></div></div><div class="card section"><div class="split"><div><h2>Store price matrix</h2><div class="small">“Need” is the comparison quantity for this month. Change any price or package amount after checking the store.</div></div><button class="btn" id="addStoreItem">+ Product</button></div><div class="tablewrap section"><table class="priceMatrix"><thead id="storeMatrixHead"></thead><tbody id="storeMatrixBody"></tbody></table></div></div><div class="card section"><div class="split"><div><h2>UPC / GTIN product sources</h2><div class="small">Look up the barcode once, then reuse the same product identity across inventory, recipes, prices and coupons.</div></div></div><div class="tools section"><input class="field" id="barcodeLookup" inputmode="numeric" placeholder="Scan or type UPC / GTIN"><button class="btn primary" id="lookupBarcode">Look up product</button></div><div class="small section" id="productLookupStatus"></div><div id="productLookupResult" class="section"></div><div class="notice section"><b>Recall + warning check</b><div class="small">Product lookups check FDA recall enforcement data and attempt a USDA FSIS match for meat, poultry and processed egg products. A match is treated as a candidate until UPC/lot/date and distribution details are verified.</div><div class="tools section"><a class="btn" href="https://www.fda.gov/food/recalls-outbreaks-emergencies/alerts-advisories-safety-information" target="_blank" rel="noopener">FDA alerts + advisories</a><a class="btn" href="https://www.fsis.usda.gov/recalls" target="_blank" rel="noopener">USDA FSIS recalls</a></div></div><div id="productSafety"></div><h3 class="section">Cached sourced products</h3><div id="productCatalog"></div></div>
<div class="card section"><div class="split"><div><h2>Coupons + promotions</h2><div class="small">Deals stay separate from shelf price. They can target a UPC or a product name, and recipe shopping automatically applies active matches to cost estimates.</div></div><button class="btn" id="addDeal">+ Coupon / deal</button></div><div id="dealRows" class="section"></div></div>
<div class="card section"><div class="split"><h2>Monthly essentials</h2><button class="btn" id="addEssential">+ Essential</button></div><div class="tablewrap section"><table><thead><tr><th>Item</th><th>Qty</th><th>Unit</th><th>Price</th><th>Total</th><th></th></tr></thead><tbody id="essentialRows"></tbody></table></div></div></section>
<section class="view" data-v="recipes"><div class="panel"><div class="split"><div><div class="eyebrow">Recipe Book</div><h2>Live recipe engine</h2><p class="muted">Recipes connect to inventory, UPC-backed products, current store prices and coupons. Keep each imported recipe's source and license attached to it.</p></div><div class="stat"><small>Recipes</small><div class="value blueText" id="recipeCount"></div></div></div><div class="tools section"><input class="field" id="recipeBookSearch" placeholder="Find a recipe or ingredient"><a class="btn" href="https://www.nutrition.gov/recipes" target="_blank" rel="noopener">Nutrition.gov</a><a class="btn" href="https://www.myplate.gov/myplate-kitchen/recipes" target="_blank" rel="noopener">MyPlate</a><a class="btn" href="https://based.cooking/" target="_blank" rel="noopener">Public-domain recipes</a></div></div>
<div class="card section"><div class="split"><div><h2>Best fit from what you have</h2><div class="small">Ranks recipes by ingredients already on hand, missing price data and current coupon-adjusted purchase cost.</div></div></div><div id="recipeIntelligence" class="section"></div></div>
<div class="card section"><div class="split"><div><h2>Add a sourced recipe</h2><div class="small">One ingredient per line: ingredient | quantity | unit.</div></div><button class="btn primary" id="addRecipeBuilder">Add recipe</button></div><div class="grid g2 section"><label><span>Recipe title</span><input class="field" id="recipeTitle"></label><label><span>Servings</span><input class="field" id="recipeServings" type="number" min="1" value="4"></label><label><span>Source name</span><input class="field" id="recipeSourceName" placeholder="USDA MyPlate, household, etc."></label><label><span>Source URL</span><input class="field" id="recipeSourceUrl" type="url"></label><label><span>License</span><select class="field" id="recipeLicense"><option>Public domain / CC0</option><option>CC BY</option><option>CC BY-SA</option><option>Household</option><option>Link only / do not republish text</option></select></label><label><span>Tags</span><input class="field" id="recipeTags" placeholder="protein, calcium, cheap"></label><label><span>Appliances</span><input class="field" id="recipeAppliances" placeholder="stove, oven, air fryer"></label></div><label class="section"><span>Ingredients</span><textarea class="field" id="recipeIngredients" rows="6" placeholder="chicken | 1.5 | lb&#10;rice | 12 | oz"></textarea></label><label class="section"><span>Instructions</span><textarea class="field" id="recipeInstructions" rows="5"></textarea></label></div>
<div class="grid g2 section" id="recipeBook"></div></section>
...
<section class="view" data-v="groceries"><div class="panel"><div class="split"><div><div class="eyebrow">Shopping workflow</div><h2>List → Cart → History</h2><div class="small">Inventory warnings only move here when you choose Add to list. Brands live in recommendations and purchase history; inventory stays generic.</div></div><div class="stat"><small>In cart</small><div class="value greenText" id="cartCount"></div></div></div><div class="shopTabs section"><button class="btn shopTab" data-shop-tab="list">Shopping list</button><button class="btn shopTab" data-shop-tab="cart">Cart</button><button class="btn shopTab" data-shop-tab="history">History</button></div><div class="grid g4 stats section" id="shoppingWorkflowStats"></div></div>
<div class="shopPane" data-shop-pane="list"><div class="card section"><div class="split"><div><h2>Shopping list</h2><div class="small">Sort by urgency, aisle, or type. Favorite products come from your purchase history; acceptable alternatives appear beside them when available.</div></div><label style="min-width:180px"><span>Sort</span><select class="field" id="shoppingSort"><option value="need">Need</option><option value="aisle">Aisle</option><option value="type">Type</option></select></label></div><div id="shoppingListRows" class="section"></div></div><div class="card section"><div class="split"><div><h2>Meal + monthly-plan needs</h2><div class="small">These are still calculated from recipes and monthly essentials. Inventory threshold items are not added here until you explicitly add them to the shopping workflow.</div></div></div><div id="plannerNeeds" class="section"></div></div></div>
<div class="shopPane" data-shop-pane="cart"><div class="card section"><div class="split"><div><h2>Cart</h2><div class="small">Use − / + or type the quantity. Replace the product name with what you actually found. When everything is correct, mark it Purchased.</div></div></div><div id="cartRows" class="section"></div></div></div>
<div class="shopPane" data-shop-pane="history"><div class="card section"><div class="split"><div><h2>Purchase history</h2><div class="small">Update price, store, product name and rating here. Favorite products become the primary recommendation next time; Acceptable products can become the alternative.</div></div></div><div id="purchaseHistoryRows" class="section"></div></div></div>
</section>
...
let invOnHand=state.inventory.filter(i=>n(i.qty)>0).length,invWarn=state.inventory.filter(inventoryWarning).length,invListed=state.inventory.filter(i=>i.shoppingState==="list"||i.shoppingState==="cart").length,invCats=new Set(state.inventory.map(i=>i.category).filter(Boolean)).size;
$("#inventoryStats").innerHTML=[["Items",state.inventory.length],["On hand",invOnHand],["Warnings",invWarn],["List / cart",invListed]].map(x=>`<div class="stat"><small>${x[0]}</small><div class="value">${x[1]}</div></div>`).join("");
$("#inventoryRows").innerHTML=inventoryWorkflowRows().map(({item:i,index:k})=>{let warn=inventoryWarning(i),stateLabel=i.shoppingState==="list"?"On list":i.shoppingState==="cart"?"In cart":warn?"⚠ LOW":"—";return`<tr class="${warn?"inventoryWarningRow":""}"><td><div class="tools"><button class="btn invMinus" data-i="${k}">−</button><input class="field invQty" style="width:78px" data-i="${k}" type="number" min="0" step="1" value="${i.qty??0}"><button class="btn invPlus" data-i="${k}">+</button></div></td><td><input class="field invMin" style="width:90px" data-i="${k}" type="number" min="0" step="1" value="${i.minQty??0}"></td><td><b class="${warn?"amberText":""}">${stateLabel}</b></td><td><input class="field invName" data-i="${k}" value="${esc(i.name||"")}"></td><td><input class="field invCat" data-i="${k}" value="${esc(i.category||"Household")}"></td><td><input class="field invAisle" data-i="${k}" value="${esc(i.aisle||"")}" placeholder="e.g. Dental"></td><td>${warn?`<label class="tools"><input class="check invAddList" data-i="${k}" type="checkbox"> Add</label>`:i.shoppingState==="list"?"✓ List":i.shoppingState==="cart"?"✓ Cart":"—"}</td><td><button class="btn danger invRemove" data-i="${k}">×</button></td></tr>`}).join("")||`<tr><td colspan="8"><div class="empty">No inventory items yet. Add generic items such as toothpaste, shampoo, cereal, dog food, or detergent.</div></td></tr>`;
...
$("#recipeCount").textContent=fullCookbook().length;renderProductSources();renderRecipeIntelligence();
let rbq=($("#recipeBookSearch")?.value||"").trim().toLowerCase(),rb=fullCookbook().filter(r=>!rbq||r.name.toLowerCase().includes(rbq)||(r.ing||[]).some(i=>String(i[0]).toLowerCase().includes(rbq)));
$("#recipeBook").innerHTML=rb.map(r=>{let ex=recipeExpected(r),live=recipeLiveCost(r);return '<div class="card"><div class="split"><div><h3>'+esc(r.name)+'</h3><div class="small">'+r.servings+' servings · '+esc(r.sourceName||"Household recipe")+(r.sourceLicense?' · '+esc(r.sourceLicense):'')+'</div></div><div class="stat"><small>Need to buy</small><div class="value">'+money(live.known)+(live.missing?' + ?':'')+'</div></div></div><div class="section">'+(r.ing||[]).map(i=>{let have=inventoryQty(i[0],i[2]),buy=Math.max(0,n(i[1])-have),v=buy?ingredientExpected(i[0],buy,i[2]):0,d=dealForIngredient(i[0],i[2]);return '<div class="kpi"><span>'+esc(i[0])+' · '+i[1]+' '+esc(i[2])+' · '+(buy?'buy '+buy:'on hand ✓')+(d?' · coupon/deal ✓':'')+'</span><b>'+(buy?(v==null?'price needed':money(v)):'$0.00')+'</b></div>'}).join('')+'</div><div class="small section">'+(ex.complete?'Current store prices and active deals are linked.':'Unpriced ingredients keep the estimate incomplete.')+'</div><div class="tools section"><button class="btn primary recipePlan" data-id="'+r.id+'">'+(planEntry(r.id)?'Planned ✓':'Plan meal')+'</button>'+(r.sourceUrl?'<a class="btn" href="'+esc(r.sourceUrl)+'" target="_blank" rel="noopener">Source</a>':'<a class="btn" href="'+search(r.q)+'" target="_blank" rel="noopener">Recipe search</a>')+'</div></div>'}).join("")||'<div class="empty">No matching recipes.</div>';
...
if($("#lookupBarcode"))$("#lookupBarcode").onclick=lookupProductByBarcode;
$(".catalogPrice").forEach(x=>x.onclick=()=>addCatalogProductToPrices(x.dataset.gtin,$("#productIngredientAlias")?.value||""));
$(".catalogInventory").forEach(x=>x.onclick=()=>addCatalogProductToInventory(x.dataset.gtin));
$(".catalogSafety").forEach(x=>x.onclick=async()=>{let p=productByGtin(x.dataset.gtin);if(p){let s=$("#productLookupStatus");if(s)s.textContent="Checking official safety sources…";await checkProductSafety(p);s=$("#productLookupStatus");if(s)s.textContent="Safety check completed."}});
$(".catalogRemove").forEach(x=>x.onclick=()=>{state.catalogProducts=state.catalogProducts.filter(p=>!barcodeSame(p.gtin,x.dataset.gtin));if(state.lastProductLookup&&barcodeSame(state.lastProductLookup.gtin,x.dataset.gtin))state.lastProductLookup=null;render()});
if($("#addDeal"))$("#addDeal").onclick=()=>{state.deals.push({id:"deal-"+Date.now(),product:"",gtin:"",store:"Any",type:"amount",value:"",expires:"",sourceUrl:""});render()};
[["dealProduct","product"],["dealGtin","gtin"],["dealStore","store"],["dealType","type"],["dealValue","value"],["dealExpires","expires"],["dealSource","sourceUrl"]].forEach(([cl,k])=>$("."+cl).forEach(x=>x.onchange=()=>{let d=state.deals[+x.dataset.i];d[k]=k==="value"?(x.value===""?"":n(x.value)):x.value;render()}));
$(".dealRemove").forEach(x=>x.onclick=()=>{state.deals.splice(+x.dataset.i,1);render()});
if($("#addRecipeBuilder"))$("#addRecipeBuilder").onclick=addRecipeFromBuilder;
...
[["invName","name"],["invCat","category"],["invAisle","aisle"],["invQty","qty"],["invMin","minQty"]].forEach(([cl,k])=>$("."+cl).forEach(x=>x.onchange=()=>{let i=state.inventory[+x.dataset.i];i[k]=["qty","minQty"].includes(k)?(x.value===""?0:Math.max(0,n(x.value))):x.value;if(k==="qty")i.onHand=n(i.qty)>0;render()}));
$(".invMinus").forEach(x=>x.onclick=()=>{let i=state.inventory[+x.dataset.i];i.qty=Math.max(0,n(i.qty)-1);i.onHand=n(i.qty)>0;render()});
$(".invPlus").forEach(x=>x.onclick=()=>{let i=state.inventory[+x.dataset.i];i.qty=n(i.qty)+1;i.onHand=true;render()});
$(".invAddList").forEach(x=>x.onchange=()=>{if(x.checked)moveInventoryToList(+x.dataset.i)});
$(".invRemove").forEach(x=>x.onclick=()=>{state.inventory.splice(+x.dataset.i,1);render()});
$(".shopTab").forEach(x=>x.onclick=()=>{state.shoppingView=x.dataset.shopTab;render()});
if($("#shoppingSort"))$("#shoppingSort").onchange=e=>{state.shoppingSort=e.target.value;render()};
$(".workflowBuyQty").forEach(x=>x.onchange=()=>{state.inventory[+x.dataset.i].buyQty=Math.max(0,n(x.value));render()});
$(".toCart").forEach(x=>x.onchange=()=>{if(x.checked)moveInventoryToCart(+x.dataset.i)});
$(".workflowRemoveList").forEach(x=>x.onclick=()=>{let i=state.inventory[+x.dataset.i];if(i){i.shoppingState="";i.buyQty="";render()}});
$(".cartBack").forEach(x=>x.onclick=()=>returnCartToList(+x.dataset.i));
$(".cartMinus").forEach(x=>x.onclick=()=>{let i=state.inventory[+x.dataset.i];i.buyQty=Math.max(0,n(i.buyQty)-1);render()});
$(".cartPlus").forEach(x=>x.onclick=()=>{let i=state.inventory[+x.dataset.i];i.buyQty=n(i.buyQty)+1;render()});
$(".cartQty").forEach(x=>x.onchange=()=>{state.inventory[+x.dataset.i].buyQty=Math.max(0,n(x.value));render()});
$(".cartProduct").forEach(x=>x.onchange=()=>{state.inventory[+x.dataset.i].cartProduct=x.value;save()});
$(".cartStore").forEach(x=>x.onchange=()=>{state.inventory[+x.dataset.i].cartStore=x.value;save()});
$(".cartPrice").forEach(x=>x.onchange=()=>{state.inventory[+x.dataset.i].cartPrice=x.value===""?"":n(x.value);save()});
$(".cartPurchased").forEach(x=>x.onclick=()=>completeCartPurchase(+x.dataset.i));
[["histProduct","product"],["histStore","store"],["histPrice","price"],["histRating","rating"]].forEach(([cl,k])=>$("."+cl).forEach(x=>x.onchange=()=>{let h=state.purchaseHistory[+x.dataset.i];h[k]=k==="price"?(x.value===""?"":n(x.value)):x.value;render()}));
$(".historyBuyAgain").forEach(x=>x.onclick=()=>{let h=state.purchaseHistory[+x.dataset.i],idx=state.inventory.findIndex(i=>i.id===h.inventoryId||canon(i.name)===canon(h.item));if(idx>=0){let i=state.inventory[idx];i.shoppingState="list";i.buyQty=Math.max(1,n(h.qty)||n(i.minQty)||1);i.cartProduct=h.product||favoriteProductFor(i)||i.name;state.shoppingView="list";render()}});
...
$$(".actualInput").forEach(x=>x.onchange=()=>{let b=actualBucket();b[x.dataset.key]=x.value===""?"":n(x.value);render()});
...
async function restorePlannerRaw(raw,sourceLabel="import"){
  ...
  try{
    ...
    ["prices","essentials","inventory","cookbook","priceHistory","incomeSources","taxItems","bills","dailyExpenses","storeOffers","assets","liabilities","sinkingFunds","purchaseHistory"].forEach(k=>{if(!Array.isArray(state[k]))state[k]=[]});
...
if($("#usdaApiKey")){$("#usdaApiKey").value=localStorage.getItem(USDA_KEY_STORAGE)||"";$("#usdaApiKey").onchange=e=>{let v=e.target.value.trim();if(v)localStorage.setItem(USDA_KEY_STORAGE,v);else localStorage.removeItem(USDA_KEY_STORAGE)}}
